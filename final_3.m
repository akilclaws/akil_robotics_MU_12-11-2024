...
% Arrays to store theta_actual, q1, q2, q_error, and counts values
theta_actual_values = [];
q1_values = [];
q2_values = [];
q_error_values = [];
...
for i = 1:num_points
    ...
    Feed_forward = real((q_d) + Kp * q_error + K_i * i_error);  % Try using Kp & ki alone to simulate the robot to achieve desired position
...
% Plot theta_actual vs q1, theta_actual vs q2, and q_error
figure;
subplot(3,1,1);
plot(theta_actual_values(1, :), 'b', 'DisplayName', 'Theta 1 Actual');
hold on;
plot(q1_values, 'r', 'DisplayName', 'q1 Desired');
legend;
title('Theta 1 Actual vs q1 Desired');
xlabel('Time Step');
ylabel('Angle (degrees)');

subplot(3,1,2);
plot(theta_actual_values(2, :), 'b', 'DisplayName', 'Theta 2 Actual');
hold on;
plot(q2_values, 'r', 'DisplayName', 'q2 Desired');
legend;
title('Theta 2 Actual vs q2 Desired');
xlabel('Time Step');
ylabel('Angle (degrees)');
