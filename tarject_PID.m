...
for i = 0.1:0.1:5  
    ...
    q = (Feed_forward_P * dt)+q;
    I = (Feed_forward_I*dt)+q;
    D = (Feed_forward_D*dt)+q;
    P_I = (Feed_forward_PI*dt)+q;
    ...
    subplot(2,2,1)
    axis([-20 20 -20 20]);
    plot (Trajectx(1,:),Trajecty(1,:),'.','MarkerSize',10,'Color','r')
    hold on;
    ...
    % Store errors
    Traject3(1,yy) = real(P_End_Effectorx);
    Traject3(2,yy) = real(P_End_Effectory);
    Traject3(3,yy) = real(I_End_Effectorx);
    Traject3(4,yy) = real(I_End_Effectory);
    Traject3(5,yy) = real(D_End_Effectorx);
    Traject3(6,yy) = real(D_End_Effectory);
    Traject3(7,yy) = real(P_I_End_Effectorx);
    Traject3(8,yy) = real(P_I_End_Effectory);    
